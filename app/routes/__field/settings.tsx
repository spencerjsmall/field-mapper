...
export default function AccountSettings() {
  const { userSurveyor } = useOutletContext();

  const [formData, setFormData] = useState({
    email: userSurveyor.user.email,
    password: "",
    firstName: userSurveyor.user.firstName,
    lastName: userSurveyor.user.lastName,
  });
  ...
  return (
    <div className="grow w-full flex  bg-slate-700 flex-col items-center justify-center">
      <h1 className="text-white mb-8">Account Settings</h1>
      <Form method="post">
        <input
          type="text"
          name="surveyorId"
          value={String(userSurveyor.id)}
          className="hidden"
          readOnly
        />
        <FormField
